    ...
    return (
        <div style={{ minHeight: '100vh', position: 'relative', zIndex: 2 }}>
            ...
            <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 24px' }}>
                ...
                {error && (
                    <div style={{ padding: '16px 20px', background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: '8px', color: '#dc2626' }}>
                        ❌ {error}
                    </div>
                )}
                    ...
                    <>
                        ...
                        {filtered.length === 0 ? (
                            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                                <p style={{ fontSize: '1.125rem', color: 'var(--text-muted)', marginBottom: '12px' }}>No hospitals found</p>
                                <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>Try adjusting your filters or search term</p>
                            </div>
                        ) : (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
                                {filtered.map((h, i) => (
                                    <motion.div key={h.id || i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: i * 0.02 }} style={{
