...
export default function BookingPage() {
    ...
    const handleBook = async () => {
        ...
        try {
            ...
            // Validate required fields - Ensure doctor data is fully loaded and not null
            if (!doc) {
            ...
            // Build request body with correct field names matching backend schema
            const bookingData = {
                doctor_id: doc.numeric_id, // Use numeric_id directly (already a number)
                patient_id: user.id ? parseInt(user.id, 10) : 1, // Use 1 as default guest patient ID if not available
                date: selectedSlot.isoDate, // YYYY-MM-DD format
                slot: time24h, // HH:MM format (24-hour)
                problem_description: condition.trim(), // Must be min 5 characters
            };
            ...
            // Log request for debugging - show FULL payload
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 296px', gap: '24px' }}>
                    ...
                    <div style={{ position: 'sticky', top: '80px', height: 'fit-content', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        ...
                        <motion.button
                            ...
                            style={{ padding: '14px', background: (isBooking || !doc?.numeric_id) ? 'rgba(11,158,135,0.6)' : 'var(--accent)', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 700, fontSize: '1rem', cursor: (isBooking || !doc?.numeric_id) ? 'not-allowed' : 'pointer', fontFamily: 'Inter, sans-serif', boxShadow: '0 4px 14px rgba(11,158,135,0.35)', transition: 'background 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                            {isBooking ? (
                                <>
